...
type MaterialType = keyof typeof standardMaterials
...
export default function BeamLoadCalculatorComponent() {
  ...
  const [shearForceData, setShearForceData] = useState<Array<{ x: number; shearForce: number }>>([])
  const [bendingMomentData, setBendingMomentData] = useState<Array<{ x: number; bendingMoment: number }>>([])
  const [material, setMaterial] = useState<MaterialType>('ASTM A36 Structural Steel')
  const [customMaterial, setCustomMaterial] = useState<CustomMaterial>({...standardMaterials['Custom']})
  ...
  const beamDiagramRef = useRef<HTMLDivElement>(null)
  const shearForceDiagramRef = useRef<HTMLDivElement>(null)
  const bendingMomentDiagramRef = useRef<HTMLDivElement>(null)
            ...
            <div className="space-y-4">
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    {(Object.keys(standardMaterials) as MaterialType[]).map((mat) => (
